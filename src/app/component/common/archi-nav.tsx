...
const ArchiNav = ({ isNavbarOpen, isMenuOpen, handleNavClick, closeNavbar }: NavBarProps) => {
  ...
  return (
    <div className="md:max-w-[1298px] px-4 mx-auto">
      <nav className="flex items-center justify-between h-25 rounded-2xl sm:p-4 sticky top-4 z-[9999] backdrop-blur-md mt-4 bg-[#E3E3E3] shadow-[3.33px_1.67px_8.33px_0px_rgba(0,0,0,0.25)]">
        <Image
          src={archibuzzLogo}
          alt=""
          height={56.3}
        ...
        <div className="block lg:hidden p-[10px] rounded-md">
          ...
          <div className={`transition-transform duration-100 ease-in-out z-50 ${isNavbarOpen ? "block" : "hidden"}`} >
            {isNavbarOpen ? (
              <ul className={`${isMenuOpen ? 'block' : 'hidden'} absolute top-[80px] sm:top-[115px] md:top-[100px] rounded-[16px] right-0 bg-[#E3E3E3] py-4 w-[250px] sm:w-[282px] text-[13.33px] font-medium shadow-custom `}>
                <NavLink
                  href={"/about"}
