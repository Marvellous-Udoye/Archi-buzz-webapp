      ...
      <Link href={`/courses/${id}`}>
        <div className="bg-[#F0F0F0] flex items-center justify-center max-w-[402px] sm:py-[43px] w-full rounded-tr-[20px] rounded-tl-[20px]">
          <Image
            ...
            width={162}
            height={227}
            className="w-full h-auto object-cover h-[200px] sm:w-[162px] sm:h-[227px] rounded-tr-[20px] rounded-tl-[20px] sm:rounded-none"
            priority
          />
        </div>
      </Link>

      <section className="flex flex-col gap-4 sm:gap-5 px-4 py-5">
        <div className=" font-medium ">
          <p className="line-clamp-2 md:h-[60px] text-[20px] leading-6 sm:leading-8">{bookTitle}</p>
          <span className="text-[#808080] text-base leading-[18px] sm:leading-6">{author}</span>
        </div>
        ...
        <div className="flex lg:justify-between w-full gap-2 sm:gap-3">
          <div className="sm:max-w-[286px] w-full">
            <Button
              ...
              styles="text-white w-full bg-[#FFA500] py-4 rounded-xl active:bg-[#CC8400] transition ease duration-100ms">
              ...
              <p className="text-base font-medium leading-6">{isAddedToCart ? "Added to Cart" : "Add to Cart"}</p>
            </Button>
          </div>

          <Button
            styles="bg-[#FFDB99] rounded-xl p-[19px]"
