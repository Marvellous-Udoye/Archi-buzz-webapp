...
const CourseCard = ({ bookCover, bookTitle, author, rating, price }: ArchiCorurseProps) => {
  return (
    <div className="border-[1px] max-w-[402px] rounded-[20px] mb-8 shadow-custom transform scale-95 hover:transform md:hover:scale-[1.02] hover:shadow-[0px_8px_20px_rgba(0,0,0,0.1),_0px_16px_40px_rgba(0,0,0,0.15)] transition ease duration-100ms ">
      <div className="bg-[#F0F0F0] flex items-center justify-center max-w-[402px] sm:py-[43px] w-full rounded-tr-[20px] rounded-tl-[20px]">
        <Image
          ...
          width={162}
          height={227}
          className="w-full h-auto object-contain sm:w-[162px] sm:h-[227px] rounded-tr-[20px] rounded-tl-[20px] sm:rounded-none"
          priority
        />
      </div>

      <section className="flex flex-col gap-5 px-4 py-5">
        <div className="text-[20px] font-medium leading-8">
          <p className="line-clamp-2 md:h-[60px]">{bookTitle}</p>
          <span className="text-[#808080] text-base font-medium leading-6">{author}</span>
        </div>

        <div className="flex justify-between">
          <span className="flex items-center gap-1">
            <p className="text-[20px] font-medium leading-10">{rating}</p>
          ...
          <p className="text-[24px] font-[700] leading-12">₦{price}</p>
        </div>

        <div className="flex justify-between">
          <Button
            styles="text-white bg-[#FFA500] py-4 px-[70px] md:px-20 lg:px-[88px] rounded-xl hover:bg-[#CC8400] transition ease duration-100ms">
          ...
          <Button
            styles="bg-[#FFDB99] rounded-xl p-[19px] hover:bg-[#CCB68F] transition ease duration-100ms"
